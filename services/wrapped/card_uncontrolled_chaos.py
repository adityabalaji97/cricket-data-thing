...
def get_uncontrolled_chaos_data(
    ...
    Criteria: SR > 130, Control < 70%, Boundary% > 15%
    """
    ...
    query = f"""
        ...
        AND (pt.runs * 100.0 / pt.balls) > {min_sr}
        AND (pt.controlled_shots * 100.0 / pt.shots_with_control_data) < {max_control_pct}
        AND (pt.boundaries * 100.0 / pt.balls) > {min_boundary_pct}
    ...
    return {
        ...
        "card_subtitle": f"High SR, low control - but it works! (min {min_balls} balls)",
        "visualization_type": "scatter",
        "criteria": {
            "min_sr": min_sr,
            "max_control_pct": max_control_pct,
            "min_boundary_pct": min_boundary_pct
        },
        "players": players,
        "deep_links": {
