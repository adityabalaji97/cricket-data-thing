        ...
        const fetchInitialData = async () => {
            try {
                ...
                const [teamsResponse, playersResponse] = await Promise.all([
                    ...
                    axios.get(`${config.API_URL}/players/`)
                ]);
    ...
    useEffect(() => {
        const fetchTeamPlayers = async () => {
            if (showMatchups && mode === 0 && selectedTeam1 && selectedTeam2) {
                try {
                    const response = await axios.get(`${config.API_URL}/teams/${selectedTeam1.full_name}/${selectedTeam2.full_name}/matchups?start_date=${startDate}&end_date=${endDate}`);
