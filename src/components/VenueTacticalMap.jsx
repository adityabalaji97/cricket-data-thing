...
const VenueTacticalMap = ({
  ...
  const zoneStatsWithDerived = useMemo(() => {
    ...
    const totalBoundaries = Object.values(zoneStats).reduce((sum, zone) => sum + toNumber(zone.boundaries), 0);
    const out = {};
    for (let zone = 1; zone <= 8; zone += 1) {
      ...
      out[key] = {
        ...
        boundary_pct: totalBoundaries > 0 ? (row.boundaries * 100) / totalBoundaries : 0,
